...
public:
		...
		: pHookFunc(func), ppOrigFunc(orig), iAbsoluteAddress(absoluteAddress)
	{
		...
		size_t iFuncNameStrlen = strlen(funcName) + 1;
		...
		: pHookFunc(func), ppOrigFunc(orig)
	{
		...
		size_t iAddrStrlen = strlen(addrString) + 1;
