...
int main()
{
    ...
    g_pSquirrel->ExecuteCode("ScriptCompilerTest()");
    
    for (int i = 0; i < 125; i++) {
        if(executedOPCodes.count(i))
            spdlog::info("instruction {} executed {} times",sq_OpToString(i),executedOPCodes[i]);
        else
            spdlog::info("instruction {} was not executed",sq_OpToString(i));
